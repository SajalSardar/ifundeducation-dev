    ...
    <div class="container ">
      <div class="row fundraisers_card">
        <div class="col-xl-3 border-end">
          ...
          @else
            <img
              src="{{ Avatar::create($fundRaiserPost->title)->setBackground('#ddd')->setDimension(250)->setFontSize(16)->toBase64() }}"
              alt="{{ $fundRaiserPost->title }}">
          @endif
          ...
          <ul class="fundraisers_card_sub">
            <li> <i class="fas fa-university"></i>{{ $fundRaiserPost->user->academic_profile->university->name }}
            </li>
          </ul>
        </div>
        <div class="col-xl-5 border-end">
          <div class="text-start px-lg-4 profile_info mt-4">
            ...
            <div class="d-sm-flex justify-content-between mt-4 pt-2">
              <div class="buttons">
                <a href="{{ route('front.stripe.donate', $fundRaiserPost->slug) }}" class="donate_btn">Donate</a>
              </div>
              <div class="social_profile mt-4 mt-sm-0" id="social-links">
                <ul class="footer_social text-start">
                  ...
                  <li><a class="social-button" href="https://twitter.com/intent/tweet?url={{ Request::url() }}"><i
                        class="fab fa-twitter"></i></a></li>
                  <li><a class="social-button" href="http://www.linkedin.com/shareArticle?url={{ Request::url() }}"><i
      ...
      <div class="row fundraisers_description_area">
        <div class="col-12 ">
          ...
          <ul class="nav nav-tabs" id="singlePostTab" role="tablist">
            ...
            <li class="nav-item" role="presentation">
              <button class="nav-link" data-bs-toggle="tab" data-bs-target="#professional_experience">Professional
                Experience</button>
            </li>
          </ul>
            ...
            <div class="tab-pane fade" id="update">
              ...
              <div class="mt-4">
                @forelse ($fundRaiserPost->fundraiserupdatemessage as $message)
                  <div
                    class="alert {{ $message->message_type === 'success' ? 'alert-success' : ($message->message_type === 'danger' ? 'alert-danger' : 'alert-warning') }}"
                    role="alert">
                    {{ $message->message }}
                  </div>
                @empty
                ...
                <div class="social_profile ">
                  <ul class="footer_social text-start">
                    @if ($fundRaiserPost->user->userSocial->instagram)
                      <li><a href="{{ $fundRaiserPost->user->userSocial->instagram }}" title="Instagram"><i
                            class="fab fa-instagram"></i></a></li>
                    @endif
                    @if ($fundRaiserPost->user->userSocial->linkedin)
                      <li><a href="{{ $fundRaiserPost->user->userSocial->linkedin }}" title="Linkedin"><i
                            class="fab fa-linkedin-in"></i></a></li>
                    @endif
                    @if ($fundRaiserPost->user->userSocial->facebook)
                      <li><a href="{{ $fundRaiserPost->user->userSocial->facebook }}" title="Facebook"><i
                            class="fab fa-facebook-f"></i></a></li>
                    @endif
                    @if ($fundRaiserPost->user->userSocial->twitter)
                      <li><a href="{{ $fundRaiserPost->user->userSocial->twitter }}" title="Twitter"><i
                            class="fab fa-twitter"></i></a></li>
                    @endif
                    @if ($fundRaiserPost->user->userSocial->youtube)
                      <li><a href="{{ $fundRaiserPost->user->userSocial->youtube }}" title="youtube"><i
                            class="fab fa-youtube"></i></a></li>
                    @endif
                    @if ($fundRaiserPost->user->userSocial->tiktok)
                      <li><a href="{{ $fundRaiserPost->user->userSocial->tiktok }}" title="Tiktok"><i
                            class="fab fa-tiktok"></i></a></li>
                    @endif
                    @if ($fundRaiserPost->user->userSocial->pinterest)
                      <li><a href="{{ $fundRaiserPost->user->userSocial->pinterest }}" title="Pinterest"><i
                            class="fab fa-pinterest-p"></i></a></li>
                    @endif
                    @if ($fundRaiserPost->user->userSocial->snapchat)
                      <li><a href="{{ $fundRaiserPost->user->userSocial->snapchat }}" title="snapchat"><i
                            class="fab fa-snapchat-ghost"></i></a></li>
                    @endif
                    @if ($fundRaiserPost->user->userSocial->website)
                      <li><a href="{{ $fundRaiserPost->user->userSocial->website }}" title="Website"><i
                            class="fas fa-globe-americas"></i></a></li>
                    @endif
                  </ul>
                </div>
              </td>

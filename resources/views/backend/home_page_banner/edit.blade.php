...
@section('content')
    <div class="card mb-5 mb-xl-8">
        <form action="{{ route('dashboard.page-options.home-page-banner.update', $homePageBanner) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <!--begin::Header-->
            <div class="card-header border-0 pt-5">
                <h3 class="card-title align-items-start flex-column" style="display: inline-block;">
                    <span class="card-label fw-bolder fs-3 mb-1">Home Page Banner (Edit)</span>
                </h3>
                <div class="card-toolbar" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-trigger="hover"
                    title="" data-bs-original-title="Click to Update Home Page Banner" style="display: inline-block; float: right;">
            ...
            <div class="card-body py-3">
                <div class="d-flex flex-column mb-8 fv-row fv-plugins-icon-container">
                    ...
                    <div class="col col-lg-11" style="padding: 5px 15px">
                        @if($homePageBanner->image != 'blank')
                            <p>{{$homePageBanner->image}}</p>
                        @endif
                    </div>
                ...
                <div class="d-flex flex-column mb-8 fv-row fv-plugins-icon-container">
                    ...
                    <textarea id="title" name="title" class="form-control" rows="2" placeholder="Enter Title">{{ $homePageBanner->title }}</textarea>
                </div>
                <div class="d-flex flex-column mb-8 fv-row fv-plugins-icon-container">
                    <label class="d-flex align-items-center fs-6 fw-bold mb-2">
                        <span class="">Description</span>
                    </label>
                    <textarea name="description" class="form-control" rows="3" placeholder="Enter Description">{{ $homePageBanner->description }}</textarea>
                </div>
                <div class="d-flex flex-column mb-8 fv-row fv-plugins-icon-container">
                    <label class="d-flex align-items-center fs-6 fw-bold mb-2">
                        <span class="">Button Text</span>
                    </label>
                    <input type="text" class="form-control form-control-solid " placeholder="Enter Button Text" name="button_text" value="{{ $homePageBanner->button_text }}">
                </div>
                <div class="d-flex flex-column mb-8 fv-row fv-plugins-icon-container">
                    <label class="d-flex align-items-center fs-6 fw-bold mb-2">
                        <span class="">Button Link</span>
                    </label>
                    <input type="text" class="form-control form-control-solid " placeholder="Enter Button Link" name="button_link" value="{{ $homePageBanner->button_link }}">
                </div>
                <div class="d-flex flex-column mb-8 fv-row fv-plugins-icon-container">
                    <label class="d-flex align-items-center fs-6 fw-bold mb-2">
                    ...
                    <select name="status" class="form-control form-control-solid ">
                        <option {{$homePageBanner->status<>1?'selected':''}} value="0">Inactive</option>
                        <option {{$homePageBanner->status==1?'selected':''}} value="1">Active</option>
                    </select>
                </div>
